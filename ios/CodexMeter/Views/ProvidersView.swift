                ...
                ForEach(Array([snapshot.fiveHour, snapshot.weekly, snapshot.monthly].enumerated()), id: \.offset) { index, window in
                    if let window {
                        VStack(alignment: .leading, spacing: 8) {
                            Text([MeterL10n.text("5-hour", "5 heures"), MeterL10n.text("Weekly", "Hebdomadaire"), MeterL10n.text("Monthly", "Mensuel")][index]).font(.headline)
                            Text("\(window.remainingPercent)% " + MeterL10n.text("remaining", "restant")).font(.largeTitle.bold())
                            ProgressView(value: Double(window.usedPercent), total: 100)
                            if let reset = window.resetAt {
                                Text(MeterL10n.text("Resets ", "Réinitialisation ") + reset.formatted(.relative(presentation: .named, unitsStyle: .wide).locale(MeterL10n.language.locale)))
                                    .font(.caption).foregroundStyle(.secondary)
                            }
