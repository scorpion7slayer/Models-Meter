...
public final class UsagePaceDemoActivity extends Activity {
    ...
    protected void onCreate(Bundle state) {
        ...
        try {
            ...
            SecureTokenStore.save(this, new AuthTokens(
                    ...
                    "debug-demo-account", "demo@modelsmeter.local"));
            ...
            // Deterministic provider fixtures for visual QA, never shipped in release builds.
            for (Provider provider : Provider.values()) {
                if (provider == Provider.CHATGPT) continue;
                ProviderCredentials.save(this, provider, new org.json.JSONObject().put("session", "debug-demo"));
                getSharedPreferences("models_meter_providers", 0).edit()
                        .putString(provider.id + "_usage", snapshot.toJson().toString())
                        .putString(provider.id + "_models", ModelCatalogStore.load(this).toJson().toString()).apply();
            }
            ProviderRepository.select(this, Provider.from(getIntent().getStringExtra("provider")));
            String language = getIntent().getStringExtra("language");
            if (language != null) L10n.select(this, language);
            ...
            if (getIntent().getBooleanExtra("validate_models_widget", false)) validateModelsWidgetLayouts();
            if (getIntent().getBooleanExtra("open_models_widget", false)) {
                int widgetId = 4242;
                ProviderRepository.setWidgetProvider(this, widgetId, ProviderRepository.selected(this));
                startActivity(new Intent(this, ModelsWidgetConfigActivity.class)
    ...
    private void validateModelsWidgetLayouts() {
        int previousCount = 0;
        for (int height : new int[]{40, 56, 64, 100, 110, 180, 300, 500}) {
            android.widget.FrameLayout host = new android.widget.FrameLayout(this);
            android.view.View widget = LatestModelsWidget.buildViews(this, -1, height, Provider.CHATGPT).apply(this, host);
            widget.measure(android.view.View.MeasureSpec.makeMeasureSpec(Ui.dp(this, 180), android.view.View.MeasureSpec.EXACTLY),
                    android.view.View.MeasureSpec.makeMeasureSpec(Ui.dp(this, height), android.view.View.MeasureSpec.EXACTLY));
            widget.layout(0, 0, widget.getMeasuredWidth(), widget.getMeasuredHeight());
            android.widget.LinearLayout rows = widget.findViewById(R.id.latest_models_rows);
            if (rows.getChildCount() < previousCount) throw new AssertionError("Widget loses models when enlarged at " + height);
            for (int i = 0; i < rows.getChildCount(); i++) {
                android.view.View row = rows.getChildAt(i);
                if (row.getTop() < 0 || row.getBottom() > rows.getHeight())
                    throw new AssertionError("Clipped widget row at " + height);
            }
            previousCount = rows.getChildCount();
        }
        android.util.Log.i("ModelsMeterUiQA", "Widget layouts passed: 40, 56, 64, 100, 110, 180, 300, 500dp");
    }
