    ...
    static boolean post(Context context, Provider provider, int id, String title, String body) {
        ...
        try {
            manager.notify(provider.id, id, new android.app.Notification.Builder(context, channel)
                    .setSmallIcon(R.drawable.ic_notification).setContentTitle(dev.scorpion7slayer.modelsmeter.Translations.t(provider.label + " · " + title))
                    .setContentText(dev.scorpion7slayer.modelsmeter.Translations.t(body)).setStyle(new android.app.Notification.BigTextStyle().bigText(dev.scorpion7slayer.modelsmeter.Translations.t(body)))
