package dev.scorpion7slayer.modelsmeter.wear;
...
import dev.scorpion7slayer.modelsmeter.AppPreferences;
import dev.scorpion7slayer.modelsmeter.DiagnosticLog;
import dev.scorpion7slayer.modelsmeter.NowBarManager;
import dev.scorpion7slayer.modelsmeter.NowBarPreferences;
import dev.scorpion7slayer.modelsmeter.RefreshScheduler;
import dev.scorpion7slayer.modelsmeter.SecureTokenStore;
import dev.scorpion7slayer.modelsmeter.UsageSnapshot;
import dev.scorpion7slayer.modelsmeter.UsagePacePreferences;
...
public final class PhoneWearSync {
    ...
    private static final String PREFS = "models_meter_wear_sync_v1";
    ...
    public static void pushProviders(Context context) {
        try {
            ...
            for (dev.scorpion7slayer.modelsmeter.Provider provider : dev.scorpion7slayer.modelsmeter.Provider.values()) {
                org.json.JSONObject item = new org.json.JSONObject().put("connected",
                        dev.scorpion7slayer.modelsmeter.ProviderRepository.connected(context, provider));
                UsageSnapshot usage = dev.scorpion7slayer.modelsmeter.ProviderRepository.usage(context, provider);
                if (usage != null) item.put("usage", usage.toJson());
                dev.scorpion7slayer.modelsmeter.ModelCatalogSnapshot catalog = dev.scorpion7slayer.modelsmeter.ProviderRepository.catalog(context, provider);
                if (catalog != null) {
                    org.json.JSONArray models = new org.json.JSONArray();
                    for (dev.scorpion7slayer.modelsmeter.CodexModelCatalog.Model model : catalog.models)
