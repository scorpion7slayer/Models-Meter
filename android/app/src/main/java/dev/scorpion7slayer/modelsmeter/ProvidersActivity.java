...
public final class ProvidersActivity extends AppCompatActivity {
        ...
        for (Provider provider : Provider.values()) {
            ...
            content.addView(Ui.actionRow(this, provider.label,
                    ...
                    R.drawable.ic_models_meter, view -> {
                        ...
                        if (connected) new AlertDialog.Builder(this).setTitle(dev.scorpion7slayer.modelsmeter.Translations.t(provider.label))
    ...
    private void connect(Provider provider) {
        ...
        save.setOnClickListener(view -> {
            ...
            save.setEnabled(false); save.setText(dev.scorpion7slayer.modelsmeter.Translations.t(L10n.text(this, "Connecting…", "Connexion…")));
            new Thread(() -> {
                try {
                    ...
                    runOnUiThread(() -> { if (!isDestroyed()) { credential.setText(dev.scorpion7slayer.modelsmeter.Translations.t("")); modelKey.setText(dev.scorpion7slayer.modelsmeter.Translations.t("")); build(); } });
                } catch (Exception error) {
                    runOnUiThread(() -> { if (!isDestroyed()) {
                        save.setEnabled(true); save.setText(dev.scorpion7slayer.modelsmeter.Translations.t(L10n.text(this, "Connect", "Connecter")));
                        new AlertDialog.Builder(this).setTitle(dev.scorpion7slayer.modelsmeter.Translations.t(provider.label)).setMessage(dev.scorpion7slayer.modelsmeter.Translations.t(error.getMessage()))
    ...
    private EditText field(String hint) {
        EditText field = new EditText(this); field.setHint(dev.scorpion7slayer.modelsmeter.Translations.t(hint)); field.setSingleLine(true);
